...
Эти лимиты снова стерегут ровно то, для чего заведены — быстрое падение: опечатка в `.env`,
ошибка импорта. Отсутствующий модем их больше не тратит: старт не ждёт устройство и не выходит,
если его нет, а линк поднимает фоновая задача, которая пробует, пока процесс жив. Раньше
вынутый из USB модем означал бесконечную череду перезапусков, заканчивавшуюся `failed` и
полностью недоступной админкой.
