# delivery-dispatch Specification

## Purpose
TBD - created by archiving change add-delivery-dispatch. Update Purpose after archive.
## Requirements
...
- **WHEN** an operator saves `[{"app_id":"app1","webhook_url":" https://example.com/webhooks/sms-gate/delivery ","bearer":"t"}]`
- **THEN** the route is stored with the surrounding whitespace removed
- **AND** delivery notifications for app `app1` POST to `https://example.com/webhooks/sms-gate/delivery`

#### Scenario: A route is saved without a scheme
- **WHEN** an operator saves a route whose `webhook_url` is `example.com/webhooks/sms-gate/delivery`
...
- **WHEN** twenty messages for app `app1` change status while its webhook returns 500
