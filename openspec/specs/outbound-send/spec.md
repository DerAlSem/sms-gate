...
then promoted with the operator. Changes `add-send-retries` and `add-send-failure-recovery` archived into it the same day. Every requirement carries a status tag: only `normative`
