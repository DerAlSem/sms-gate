...
- [x] 2.1 Extract the route validation and stripping added for `inbound_dispatch` in
      `app/settings_store.py` into a shared helper parameterized by the key field
      (`prefix` vs `app_id`).
- [x] 2.2 Add the `delivery_dispatch` spec key (type `json`, section "Inbound dispatch"
      renamed to "Dispatch", default empty) and its validation.
- [x] 2.3 Add a `delivery_dispatch_parsed` accessor mirroring `inbound_dispatch_parsed`,
      dropping entries without an `app_id` or `webhook_url`.
- [x] 2.4 Extend the `inbound_dispatch` JSON-editor branch in
      `app/admin/templates/settings.html` to cover `delivery_dispatch`; add the Russian
      translation for the new description.
- [x] 2.5 Tests: save/normalize/reject routes; an app with no route neither posts nor
      alerts (the silent-by-design case from D1).

## 3. Migration

- [x] 3.1 Add a nullable `messages.resent_from INTEGER REFERENCES messages(id)` column
      in `app/db/migrate.py`, following the existing idempotent style.
- [x] 3.2 Set it in the admin Resend handler (`create_message` gains the source id) and
      expose it on the row read by the dispatcher.
- [x] 3.3 Test: an API-created message has `resent_from` NULL; a re-sent one carries the
      source id; the migration is idempotent on an existing DB.

## 4. The sender

- [x] 4.1 `app/modem/delivery_dispatch.py`: `find_route(app_id)`, `deliver(route, payload)`
...
- [x] 4.2 Build the body: always `id`, `status`, `error`, `occurred_at` (ISO-8601 UTC);
      `resent_from` only when set.
- [x] 4.3 Fire the `dispatch_error` alert on total failure, deduped on the url, with the
      app id, message id and status in the text.
- [x] 4.4 Spawn detached with a strong task reference (reuse the `_spawn_dispatch`
      pattern and its GC note).
- [x] 4.5 Tests: success is silent; non-2xx and transport errors alert once; the bearer
...
- [x] 5.1 Enumerate every writer of `messages.status`: `mark_sent`, `mark_failed`,
      `set_message_delivered`, `set_message_failed`, and the expire sweep.
- [x] 5.2 Call `dispatch_delivery` from each, reading `app_id` and `resent_from` from the
      message row. `expire_stale_messages` is the one bulk writer: it now `UPDATE … RETURNING id`
      (SQLite ≥ 3.35), so every expired message notifies instead of the batch going silent.
- [x] 5.2a `expired` is not terminal — `find_message_parts_by_ref` accepts it, so a late
      report moves `expired → delivered` and notifies twice. Test that sequence.
- [x] 5.3 Conformance test (design D7): enumerate `UPDATE messages SET status` sites and
      the `queries.py` helpers owning them, and fail when one is not covered.
- [x] 5.4 Test the multipart case end to end: a two-part message notifies `delivered`
      once, after the second part's report — not twice, not on the first.
- [x] 5.5 Test that a delivery report arriving while `sent` is still retrying is not
      queued behind it (D6).
