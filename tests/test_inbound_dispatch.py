...
def test_deliver_sends_bearer_and_payload(monkeypatch, env):
    ...
    asyncio.run(dispatch.deliver(_route(), {"phone": "+79991234567", "text": "GMP 8U5Z6G"}))
    url, payload, headers = posted[0]
    assert url == "https://x.test/hook"
    assert payload == {"phone": "+79991234567", "text": "GMP 8U5Z6G"}
    assert headers["Authorization"] == "Bearer tok"


def test_failed_dispatch_alerts_the_operator(monkeypatch, env):
    _fake_client(monkeypatch, lambda *a: FakeResponse(401, "unauthorized"))
    ok = asyncio.run(dispatch.dispatch_inbound("+79991234567", "GMP 8U5Z6G"))
    assert ok is False
...
def test_successful_dispatch_is_silent(monkeypatch, env):
    _fake_client(monkeypatch, lambda *a: FakeResponse(200))
    assert asyncio.run(dispatch.dispatch_inbound("+79991234567", "GMP 8U5Z6G")) is True
    assert env.calls == []
...
def test_unrouted_sms_is_silent(monkeypatch, env):
    ...
    _fake_client(monkeypatch, lambda *a: FakeResponse(200))
    assert asyncio.run(dispatch.dispatch_inbound("+79991234567", "привет как дела")) is False
    assert env.calls == []
...
def test_alert_can_be_switched_off(monkeypatch, env):
    ...
    asyncio.run(dispatch.dispatch_inbound("+79991234567", "GMP 8U5Z6G"))
    assert env.calls == []
